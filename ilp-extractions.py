...
def add_coupled_constraints_to_dict(coupled_constraints, token_semantictype_weight_dict, token_semantictype_index_dict):
...
def combine_values(old_value, new_value, logic = 'max'):
...
def update_weights_dictionary(token_semantictype_source_weight_dict, token_semantictype_source_index_dict, tokens_with_semantic_types, source, input_index, weight_of_extraction = 1.0):
    """
            ...
            for semantic_type in semantic_types:
                ...
                if(offset == 0):
                    if((value, semantic_type_name, source) not in token_semantictype_source_weight_dict):
                        ...
                        token_semantictype_source_weight_dict[(value, semantic_type_name, source)] = combine_values(old_value, semantic_probability*weight_of_extraction)
...
coupled_constraints = [
{
    "from":"city",
    "to":"country",
    "dictionary_file":CITY_COUNTRY_DICTIONARY 
},
{
    "from":"city",
    "to":"state",
    "dictionary_file":CITY_STATE_DICTIONARY
},
{
    "from":"state",
    "to":"country",
    "dictionary_file":STATE_COUNTRY_DICTIONARY
}
]

def formulate_ILP(tokens_input, coupled_constraints):

    # Read the dictionaries of the coupled constraints
    for constraint in coupled_constraints:
        print constraint['from'] + " " + constraint['to']
        constraint['dictionary'] = get_dict(constraint['dictionary_file'])
    ...
    for index, token_input in enumerate(tokens_input):
        update_weights_dictionary(token_semantictype_source_weight_dict, token_semantictype_source_index_dict, token_input['tokens'], token_input['source'], index, token_input['weight'])
    ...
    add_coupled_constraints_to_dict(coupled_constraints, token_semantictype_weight_dict, token_semantictype_index_dict)
...
# city_country_dict = get_dict(CITY_COUNTRY_DICTIONARY)
# city_state_dict = get_dict(CITY_STATE_DICTIONARY)

# token_semantictype_source_weight_dict = tupledict()
# update_weights_dictionary(token_semantictype_source_weight_dict, tokens_from_text, 'text', TEXT_WEIGHT)
# update_weights_dictionary(token_semantictype_source_weight_dict, tokens_from_title, 'title', TITLE_WEIGHT)


# print token_semantictype_source_weight_dict
...
# # Handling the city country relationships

# city_tokens = set()
# country_tokens = set()
# for (token, semantic_type, extra_info) in token_semantictype_weight_dict:
#     if(semantic_type == 'city'):
#         city_tokens.add(token)
#         countries_for_city = set(['NA'])
#         if(token in city_country_dict):
#             countries_for_city = set(city_country_dict[token])
#         for country in countries_for_city:
#             print "Added "+token+" "+country
#             # Adding country in the extra_info field
#             token_semantictype_weight_dict[token, 'city_country', country] = 0 # Added weight as 0 so that it does not come in the objective function
#             if((country, 'country', '') not in token_semantictype_weight_dict):
#                 # This is a new country that is introduced because of the cities
#                 token_semantictype_weight_dict[country, 'country',''] = 0 #Adding weight as 0 so that it does not come in the objective function
#                 country_tokens.add(country)
#     if(semantic_type == 'country'):
#         country_tokens.add(token)
...
#     m.addConstr((extractions.sum(city, 'city_country', '*') - extractions.sum(city, 'city', '*') == 0), "city_in_one_country")

# # Adding the constraint that a city can be chosen only from the country which is chosen
# # For all city_country variables, the value is <= the value of the country variable

# for country in country_tokens:
#     m.addConstrs( (city_country_var <= extractions.sum(country, 'country', '') for city_country_var in extractions.select('*', 'city_country', country)), "cities_from_only_the_country_chosen")
